...
def check_my_city(city):
  stations_nb=[]
  zip_code=[]
  for single_velib in velib:
    if city.lower() in single_velib['city'].lower():
      stations_nb.append(city)
      zip_code.append(single_velib['zip'])

  if len(stations_nb) == 0:
    return "Sorry! No station for your city has been found!"
  else:
    return {"stations_nb": len(stations_nb), "zip_code": zip_code, "city": city}
